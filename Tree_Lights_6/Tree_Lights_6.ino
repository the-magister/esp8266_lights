...
#define FASTLED_ALLOW_INTERRUPTS 0 // May cause occasional ESP resets, use with caution
//#define FASTLED_INTERRUPT_RETRY_COUNT 0
//#define FASTLED.setMaxRefreshRate(100); // Limit refresh rate
//#define FASTLED_ALL_PINS_HARDWARE_SPI
//#define ESP8266_SPI

#define FASTLED_ESP8266_RAW_PIN_ORDER
#include <FastLED.h> // 3.1.6 is last known to produce no flicker.
...
// NTP time
WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "pool.ntp.org", -8UL * 3600UL, 60000);
...
struct Settings {
  ...
  byte bright;
  byte segments;
  byte onHour1, offHour1;
  byte onHour2, offHour2;
};
Settings s;
...
// A red and white striped palette
// "CRGB::Gray" is used as white to keep the brightness more uniform.
const TProgmemRGBPalette16 WhiteRed_p FL_PROGMEM =
...
// A red and white striped palette
// "CRGB::Gray" is used as white to keep the brightness more uniform.
const TProgmemRGBPalette16 WhiteGreen_p FL_PROGMEM =
{ CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::Green,  CRGB::Green,  CRGB::Green,  CRGB::Green
...
// A white and red striped palette
// "CRGB::Gray" is used as white to keep the brightness more uniform.
const TProgmemRGBPalette16 WhiteBlue_p FL_PROGMEM =
{ CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::Blue, CRGB::Blue, CRGB::Blue, CRGB::Blue
...
// A red and white striped palette
// "CRGB::Gray" is used as white to keep the brightness more uniform.
const TProgmemRGBPalette16 WhiteGreenBerry_p FL_PROGMEM =
{ CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::FairyLight, Holly_Red, CRGB::FairyLight, CRGB::FairyLight,
  CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight, CRGB::FairyLight,
  Holly_Green,  Holly_Green,  Holly_Green,  Holly_Green
};
...
void returnForm() {
  ...
  message += radioInput("Color", "58", s.color == 58, "WhiteRed");
  message += radioInput("Color", "59", s.color == 59, "WhiteGreen");
  message += radioInput("Color", "60", s.color == 60, "WhiteBlue");
  message += radioInput("Color", "61", s.color == 61, "WhiteGreenBerry");
  message += "<br><br>";
  ...
  message += "<h4>Timers</h4>";
  message += "On Hour 1 ";
  message += numberInput("OnHour1", s.onHour1, 0, 24);
  message += "  Off Hour 1 ";
  message += numberInput("OffHour1", s.offHour1, 0, 24);
  message += "<br><br>";

  message += "On Hour 2 ";
  message += numberInput("OnHour2", s.onHour2, 0, 24);
  message += "  Off Hour 2 ";
  message += numberInput("OffHour2", s.offHour2, 0, 24);
...
void handleSubmit() {
  ...
  s.onHour1 = server.arg("OnHour1").toInt();
  s.offHour1 = server.arg("OffHour1").toInt();
  s.onHour2 = server.arg("OnHour2").toInt();
  s.offHour2 = server.arg("OffHour2").toInt();

  // display
  Serial << F("Request: color=") << s.color << F(" sparkles=") << s.sparkles;
  Serial << F(" brightness=") << s.bright << F(" segments=") << s.segments;
  Serial << F(" onHour1=") << s.onHour1 << F(" offHour1=") << s.offHour1;
  Serial << F(" onHour2=") << s.onHour2 << F(" offHour2=") << s.offHour2 << endl;
...
void updateFromSettings() {
  ...
  switch ( s.color ) {
    ...
    case 58: currentPalette = WhiteRed_p; break;
    case 59: currentPalette = WhiteGreen_p; break;
    case 60: currentPalette = WhiteBlue_p; break;
    case 61: currentPalette = WhiteGreenBerry_p; break;
    // last?
    case 62: currentPalette = CheerLights_p; break;
  }
...
void heartBeat() {
  ...
  if ( s.color == 1 || s.color == 60 ) newColor = getCheerLightsColor();
...
void checkAlarms() {
  Serial << "Getting time...  ";
  timeClient.update();
  Serial << F("NTP Time: ") << timeClient.getFormattedTime() << endl;

  static byte lastHour = 99;
  
  byte thisHour = timeClient.getHours();
/*  
  static byte thisHour = 4;
  thisHour++;
  if( thisHour>=24 ) thisHour=0;
  Serial << "Hour=" << thisHour << endl;
*/
  
  if( lastHour != thisHour ) {
    boolean on1 = thisHour >= s.onHour1 && thisHour < s.offHour1;
    boolean on2 = thisHour >= s.onHour2 && thisHour < s.offHour2;
    boolean onAll = on1 || on2;
    // if we should be on but are off.
    if( onAll && s.color == 0 ) alarmOn();
    // if we should not be on but are on.
    if( !onAll && s.color !=0 ) alarmOff();
  }
  lastHour = thisHour;
...
void alarmOff() {
  s.color = 0;
  Serial << timeClient.getFormattedTime() << F(".  Alarm Off.  color=") << s.color << endl;
  updateFromSettings();
}
void alarmOn() {
  s.color = random(50, 60 + 1);
  Serial << timeClient.getFormattedTime() << F(".  Alarm On.  color=") << s.color << endl;
  updateFromSettings();
}
...
void setup(void) {
  ...
  // create timer connection to NIST
  timeClient.begin();
  delay(100);
  timeClient.update();  // first update seems to be junk

  // update to last settings
  updateFromSettings();
...
void loop(void) {
  ...
  static Metro heartbeat(15UL*1000UL);
  if ( heartbeat.check() ) {
    heartBeat();
  }

  static Metro alarms(3UL*60UL*1000UL);
  if ( alarms.check() ) {
    checkAlarms();
  }
}
